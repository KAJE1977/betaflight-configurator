...
TuningSliders.calculateNewPids = function() {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        ...
        FC.TUNING_SLIDERS.slider_master_multiplier = Math.round((this.sliderMasterMultiplier * 100)/5)*5;
        FC.TUNING_SLIDERS.slider_roll_pitch_ratio = Math.round((this.sliderRollPitchRatio * 100)/5)*5;
        FC.TUNING_SLIDERS.slider_i_gain = Math.round((this.sliderIGain * 100)/5)*5;
        FC.TUNING_SLIDERS.slider_pd_ratio = Math.round((this.sliderPDRatio * 100)/5)*5;
        FC.TUNING_SLIDERS.slider_pd_gain = Math.round((this.sliderPDGain * 100)/5)*5;
        FC.TUNING_SLIDERS.slider_dmin_ratio = Math.round((this.sliderDMinRatio * 100)/5)*5;
        FC.TUNING_SLIDERS.slider_ff_gain = Math.round((this.sliderFFGain * 100)/5)*5;
...
TuningSliders.calculateNewGyroFilters = function() {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        ...
        //rounds slider values to nearies multiple of 5 and passes to the FW
        FC.TUNING_SLIDERS.slider_gyro_filter_multiplier = Math.round((this.sliderGyroFilterMultiplier * 100)/5)*5;
...
TuningSliders.calculateNewDTermFilters = function() {
    ...
    if (semver.gte(FC.CONFIG.apiVersion, API_VERSION_1_44)) {
        ...
        //rounds slider values to nearies multiple of 5 and passes to the FW
        FC.TUNING_SLIDERS.slider_dterm_filter_multiplier = Math.round((this.sliderDTermFilterMultiplier * 100)/5)*5;
        this.writeFilterSliders();
    } else {
...
TuningSliders.writeFilterSliders = function () {
    ...
    .then(() => {
        ...
        if (TuningSliders.GyroSliderUnavailable || TuningSliders.DTermSliderUnavailable) {
